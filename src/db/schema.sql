-- MCQ Hedge Fund — PostgreSQL schema (canonical, idempotent)
-- Run: psql $DATABASE_URL -f src/db/schema.sql
-- Mounted as docker-entrypoint-initdb.d/01_schema.sql in docker-compose
...
CREATE INDEX IF NOT EXISTS idx_event_ledger_occurred_at
  ON event_ledger (occurred_at DESC);
CREATE INDEX IF NOT EXISTS idx_event_ledger_aggregate
  ON event_ledger (aggregate_id, seq ASC);
CREATE INDEX IF NOT EXISTS idx_event_ledger_correlation
  ON event_ledger (correlation_id) WHERE correlation_id IS NOT NULL;
...
-- ── 4. KALSHI TIME-SERIES (partitioned) ──────────────────────────────────────────
