 ...
 *
 * All responses include an `as_of` ISO timestamp so the UI can show data freshness.
 */
...
export async function getInvestorSummary(
  ...
  try {
    const portfolios = portfolioStore.allPortfolios();
    const as_of = new Date().toISOString();

    const summaries = portfolios.map((entry) => {
      const { portfolio, positions } = entry;

      const gross_market_value = positions.reduce(
        (sum, p) => sum + Math.abs(p.quantity * p.current_price),
        0
      );
      const net_pnl = positions.reduce(
        (sum, p) => sum + (p.current_price - p.avg_cost) * p.quantity,
        0
      );
      const equity = portfolio.cash + gross_market_value;
      ...
      const position_rows = positions.map((p) => ({
        ...
        pnl:           Number(((p.current_price - p.avg_cost) * p.quantity).toFixed(2)),
        pnl_pct:       p.avg_cost > 0
          ? Number((((p.current_price - p.avg_cost) / p.avg_cost) * 100).toFixed(2))
          : 0,
        weight_pct:    gross_market_value > 0
          ? Number(((Math.abs(p.quantity * p.current_price) / gross_market_value) * 100).toFixed(2))
          : 0,
      }));
      ...
      return {
        ...
        net_pnl_pct:        portfolio.ytd_start_equity > 0
          ? Number(((net_pnl / portfolio.ytd_start_equity) * 100).toFixed(2))
          : 0,
  ...
  try {
    ...
    for (const entry of portfolios) {
      for (const pos of entry.positions) {
        // Use governance score from position if available, otherwise default
        const gov_score = (pos as Record<string, unknown>)['governance_score'] as number | undefined;
        const g = typeof gov_score === 'number' ? gov_score : 0.75;
        ...
        try {
          decision = runGovernanceScorer({
            ...
            account_equity:          entry.portfolio.cash +
              entry.positions.reduce((s, p) => s + Math.abs(p.quantity * p.current_price), 0),
        ...
        // Enforcement probability curve over 12 monthly horizons
        const curve = Array.from({ length: 12 }, (_, i) => {
          const t_days = (i + 1) * 21; // ~1 month each
        ...
        governance_rows.push({
          portfolio_id:       entry.portfolio.portfolio_id,
...
export async function getInvestorTimeline(
  ...
  try {
    ...
    const timeline = events.map((e) => ({
      ...
      source:       e.source,
      // Omit raw payload from investor view — surface summary only
